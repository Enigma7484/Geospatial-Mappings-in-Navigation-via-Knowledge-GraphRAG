The system contains six main stages: trace processing, approximate map matching, feature extraction, profile construction, candidate generation, and route-candidate reranking. Figure~\ref{fig:pipeline} summarizes the data flow.
...
\begin{figure*}[t]
  ...
  $\rightarrow$ reranking and evaluation
  \end{minipage}}
  \caption{End-to-end framework. Public movement signals are converted into reconstructed route references and pseudo-history profiles. For a new origin-destination query, the system generates OSM candidates and reranks them with baseline, profile, prompt, or hybrid methods.}
  \Description{A pipeline from public movement signals through segmentation, map matching, feature extraction, profile construction, candidate generation, reranking, and evaluation.}
  \label{fig:pipeline}
\end{figure*}
...
\begin{itemize}
  ...
  \item \textbf{Prompt/SBERT:} route summaries reranked by semantic similarity to preference text.
  \item \textbf{Hybrid:} $0.75$ profile score plus $0.25$ prompt/SBERT score, matching the backend.
  \item \textbf{Learned feature ranker:} a random-forest regressor trained only on earlier query candidates to predict feature distance to the reconstructed reference.
\end{itemize}
